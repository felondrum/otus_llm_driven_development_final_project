...
const Profiles = () => {
  ...
  const loadProfiles = async () => {
    try {
      const res = await axios.get('/api/v1/admin/profiles')
  ...
  const handleDelete = async (profile) => {
    if (window.confirm(`Delete profile ${profile.user_id}?`)) {
      try {
        await axios.delete(`/api/v1/admin/profiles/${profile.user_id}`)
        loadProfiles()
      } catch (error) {
  ...
  const handleSubmit = async (data) => {
    try {
      if (editingProfile) {
        await axios.put(`/api/v1/admin/profiles/${data.user_id}`, data)
      } else {
        await axios.post('/api/v1/admin/profiles', data)
      }
      setShowForm(false)
